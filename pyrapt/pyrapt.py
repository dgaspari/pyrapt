...
def demo(wavfile_path):
    ...
    downsampling_rate = Pyrapt.calculate_downsampling_rate(original_audio[0], 500) 
    #2nd arg could be constant configurable - max Fzero
    #downsampled_audio = signal.resample(original_audio[1], int(downsampling_rate)) 
    #decimate or resample options available here:
    downsampled_audio = signal.decimate(original_audio[1], int(downsampling_rate)) 
